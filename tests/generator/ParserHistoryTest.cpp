...
TEST_F(SCXMLParserHistoryTest, DetailedHistoryStateTest)
{
    ...
    // 'on' 상태 찾기
    SCXML::Model::IStateNode *onState = model->findStateById("on");
    ...
    bool foundH1 = false;
    bool foundH2 = false;
    SCXML::Model::IStateNode *h1State = nullptr;
    SCXML::Model::IStateNode *h2State = nullptr;
    ...
    for (const auto &child : children)
    {
        if (child->getId() == "h1")
        {
            foundH1 = true;
            h1State = child.get();
            EXPECT_EQ(SCXML::Type::HISTORY, child->getType()) << "h1 should be a history state";
        ...
        else if (child->getId() == "h2")
        {
            foundH2 = true;
            h2State = child.get();
            EXPECT_EQ(SCXML::Type::HISTORY, child->getType()) << "h2 should be a history state";
    ...
    SCXML::Model::IStateNode *settingsState = model->findStateById("settings");
...
TEST_F(SCXMLParserHistoryTest, DetailedHistoryStateTest2)
{
    ...
    for (const auto &child : children)
    {
        if (child->getId() == "h1")
        {
            foundH1 = true;
            h1State = child.get();
            EXPECT_EQ(SCXML::Type::HISTORY, child->getType());
