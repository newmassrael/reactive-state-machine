...
// 데이터 모델 파싱 테스트 - 커스텀 네임스페이스와 확장 속성 테스트
TEST_F(DataModelParserTest, ParseDataModel) {
    // 상태 및 데이터 모델 항목 생성 호출 예상
    EXPECT_CALL(*mockFactory, createStateNode(testing::_, testing::_)).Times(testing::AtLeast(1));

    // 데이터 모델 항목 생성 호출 예상 - 최소 3개의 항목이 있어야 함
    EXPECT_CALL(*mockFactory, createDataModelItem(testing::_, testing::_)).Times(testing::AtLeast(3));
...
// 데이터 모델 항목 파싱 테스트 - type 속성 확장성 테스트
...
TEST_F(DataModelParserTest, DataModelBindingTest) {
  ...
  <scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0" initial="s1" binding="late" datamodel="ecmascript">
    ...
    // 상태 레벨 데이터 항목 확인
    SCXML::Model::IStateNode *s1 = lateModel->findStateById("s1");
    ASSERT_TRUE(s1 != nullptr);

    // 상태별 데이터 항목 확인 - 기존 getDataItems() 메서드 사용
    const auto &stateItems = s1->getDataItems();
    if (!stateItems.empty()) {  // 상태별 데이터 항목이 있는 경우에만 검증
...
// <donedata> 요소 파싱 테스트 - W3C 표준 준수 테스트
TEST_F(DataModelParserTest, ParseDoneData) {
    // XSD 검증 활성화 - W3C 표준 준수 테스트
    parser->setXSDValidationEnabled(true);
...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0" initial="s1">
  ...
  <final id="final">
    ...
    SCXML::Model::IStateNode *finalState = nullptr;
    if (model != nullptr) {
    ...
    EXPECT_TRUE(
        parser->hasErrors() || !parser->getErrorMessages().empty() ||
...
TEST_F(DataModelParserTest, DataModelSupport) {
    ...
    EXPECT_CALL(*mockFactory, createStateNode(testing::_, testing::_)).Times(testing::AtLeast(3));
    EXPECT_CALL(*mockFactory, createDataModelItem(testing::_, testing::_)).Times(testing::AtLeast(3));
...
TEST_F(DataModelParserTest, DataModelTypesDetailedTest) {
    ...
    SCXML::Model::IStateNode *s1InNull = nullModel->findStateById("s1");
...
// XPath 데이터 모델 지원 테스트 - 확장 데이터 모델 테스트
...
TEST_F(DataModelParserTest, DetailedDoneDataTest) {
    ...
    SCXML::Model::IStateNode *finalWithParams = model->findStateById("finalWithParams");
    ...
    SCXML::Model::IStateNode *finalWithContent = model->findStateById("finalWithContent");
...
// 시스템 변수 처리 테스트 - W3C 표준 준수 테스트
TEST_F(DataModelParserTest, SystemVariablesProcessing) {
    // XSD 검증 활성화 - W3C 표준 준수 테스트
    parser->setXSDValidationEnabled(true);

    // SystemVariables 접근 테스트
    EXPECT_CALL(*mockFactory, createStateNode(testing::_, testing::_)).Times(testing::AtLeast(2));
    EXPECT_CALL(*mockFactory, createDataModelItem(testing::_, testing::_)).Times(testing::AtLeast(4));
...
TEST_F(DataModelParserTest, InFunctionTest) {
    ...
    // s2 상태의 전환 확인
    SCXML::Model::IStateNode *s2 = model->findStateById("s2");
    ...
    SCXML::Model::IStateNode *s5 = model->findStateById("s5");
...
// 가드 조건 파싱 테스트 - 커스텀 네임스페이스 확장성 테스트
TEST_F(DataModelParserTest, ParseGuards) {
    // 가드 노드 생성 호출 예상
    EXPECT_CALL(*mockFactory, createStateNode(testing::_, testing::_)).Times(testing::AtLeast(3));  // s1, s2, s3
...
<state id="s3"/>
    ...
    // s1 상태의 전환 확인
    SCXML::Model::IStateNode *s1 = model->findStateById("s1");
    ASSERT_TRUE(s1 != nullptr);
