...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0" initial="main">
  <state id="main" initial="child1">
    ...
    SCXML::Model::IStateNode *main = model->findStateById("main");
    ASSERT_TRUE(main != nullptr);

    // 자식 상태 중에서 child1 찾기
    SCXML::Model::IStateNode *child1 = nullptr;
...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0" initial="parent">
  ...
  <state id="sibling">
    ...
    // 전환 타입 검증
    SCXML::Model::IStateNode *parentState = model->findStateById("parent");
    ...
    // 형제 상태의 전환 검사
    SCXML::Model::IStateNode *siblingState = model->findStateById("sibling");
...
TEST_F(TransitionParserTest, DetailedMultipleTargetsTest)
{
    ...
    SCXML::Model::IStateNode *main = model->findStateById("main");
    ASSERT_TRUE(main != nullptr);
    EXPECT_EQ(SCXML::Type::PARALLEL, main->getType());
