...
TEST_F(StateNodeParserTest, ComplexNestedStates)
{
  ...
  auto realParser = std::make_shared<SCXML::Parsing::DocumentParser>(realFactory);
  
  // Set up related parsers for real parser
  realParser->getStateNodeParser()->setRelatedParsers(
      realParser->getTransitionParser(),
      realParser->getActionParser(),
      realParser->getDataModelParser(), 
      realParser->getInvokeParser(), 
      realParser->getDoneDataParser());
...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0" initial="s1">
  ...
  // Debug: Print all states that were created
  if (model) {
    std::cout << "=== DEBUG: States found ===\n";
    for (const auto& stateId : {"s1", "s1.1", "s1.1.1", "s1.1.2", "s1.2", "s2"}) {
      auto state = model->findStateById(stateId);
      std::cout << "State '" << stateId << "': " << (state ? "FOUND" : "NOT FOUND") << "\n";
    }
    std::cout << "============================\n";
  }
...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0" initial="p">
  ...
  // 병렬 상태 및 자식 상태 확인
  SCXML::Model::IStateNode *p = model->findStateById("p");
  ASSERT_TRUE(p != nullptr);
  EXPECT_EQ(SCXML::Type::PARALLEL, p->getType());
...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0">
  ...
  // parent 상태 찾기
  SCXML::Model::IStateNode *parent = model->findStateById("parent");
...
TEST_F(StateNodeParserTest, MacrostepMicrostepProcessing)
{
  ...
  {
    SCXML::Model::IStateNode *state = model->findStateById(stateId);
...
TEST_F(StateNodeParserTest, ParallelStatesConflictingTransitionsTest)
{
  ...
  SCXML::Model::IStateNode *r1a = model->findStateById("r1a");
  SCXML::Model::IStateNode *r1 = model->findStateById("r1");
  ASSERT_TRUE(r1a != nullptr && r1 != nullptr);

  // r2a의 첫 번째 전환이 r2b의 두 번째 전환보다 우선순위가 높은지 확인
  SCXML::Model::IStateNode *r2a = model->findStateById("r2a");
  SCXML::Model::IStateNode *r2b = model->findStateById("r2b");
...
<scxml xmlns="http://www.w3.org/2005/07/scxml" version="1.0">
  ...
  SCXML::Model::IStateNode *state1 = model->findStateById("state1");
  ASSERT_TRUE(state1 != nullptr);
  EXPECT_EQ("state1_1", state1->getInitialState());

  // state2 찾기 (<initial> 요소 사용)
  SCXML::Model::IStateNode *state2 = model->findStateById("state2");
