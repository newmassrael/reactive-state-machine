...
public:
    ...
    void SetupDefaultBehavior() {
        ...
        // 기본 동작 정의
        ON_CALL(*this, getId()).WillByDefault(testing::Invoke([this]() -> const std::string & {
            std::cout << "MockInvokeNode::getId() called, returning: " << id_ << std::endl;
            return id_;
        }));

        ON_CALL(*this, getType()).WillByDefault(testing::Invoke([this]() -> const std::string & {
            std::cout << "MockInvokeNode::getType() called, returning: " << type_ << std::endl;
            return type_;
        }));

        ON_CALL(*this, getSrc()).WillByDefault(testing::Invoke([this]() -> const std::string & {
            std::cout << "MockInvokeNode::getSrc() called, returning: " << src_ << std::endl;
            return src_;
        }));

        ON_CALL(*this, isAutoForward()).WillByDefault(testing::Invoke([this]() -> bool {
            std::cout << "MockInvokeNode::isAutoForward() called, returning: " << (autoForward_ ? "true" : "false")
                      << std::endl;
            return autoForward_;
        }));

        ON_CALL(*this, getIdLocation()).WillByDefault(testing::Invoke([this]() -> const std::string & {
            std::cout << "MockInvokeNode::getIdLocation() called, returning: " << idLocation_ << std::endl;
            return idLocation_;
        }));

        ON_CALL(*this, getNamelist()).WillByDefault(testing::Invoke([this]() -> const std::string & {
            std::cout << "MockInvokeNode::getNamelist() called, returning: " << namelist_ << std::endl;
            return namelist_;
        }));

        ON_CALL(*this, getContent()).WillByDefault(testing::Invoke([this]() -> const std::string & {
            std::cout << "MockInvokeNode::getContent() called, returning: " << content_ << std::endl;
            return content_;
        }));

        ON_CALL(*this, getFinalize()).WillByDefault(testing::Invoke([this]() -> const std::string & {
        ...
        ON_CALL(*this, getParams())
            .WillByDefault(
                testing::Invoke([this]() -> const std::vector<std::tuple<std::string, std::string, std::string>> & {
                    std::cout << "MockInvokeNode::getParams() called, returning " << params_.size() << " params"
                              << std::endl;
                    return params_;
                }));
        ...
        ON_CALL(*this, setSrc(testing::_)).WillByDefault(testing::Invoke([this](const std::string &src) {
            std::cout << "MockInvokeNode::setSrc() called with: " << src << std::endl;
            this->src_ = src;
        }));

        ON_CALL(*this, setIdLocation(testing::_)).WillByDefault(testing::Invoke([this](const std::string &idLocation) {
            std::cout << "MockInvokeNode::setIdLocation() called with: " << idLocation << std::endl;
            this->idLocation_ = idLocation;
        }));

        ON_CALL(*this, setNamelist(testing::_)).WillByDefault(testing::Invoke([this](const std::string &namelist) {
            std::cout << "MockInvokeNode::setNamelist() called with: " << namelist << std::endl;
            this->namelist_ = namelist;
        }));

        ON_CALL(*this, setAutoForward(testing::_)).WillByDefault(testing::Invoke([this](bool autoForward) {
            std::cout << "MockInvokeNode::setAutoForward() called with: " << (autoForward ? "true" : "false")
                      << std::endl;
        ...
        ON_CALL(*this, addParam(testing::_, testing::_, testing::_))
            .WillByDefault(
                testing::Invoke([this](const std::string &name, const std::string &expr, const std::string &location) {
                    std::cout << "MockInvokeNode::addParam() called with name=" << name << ", expr=" << expr
                              << ", location=" << location << std::endl;
                    this->params_.push_back(std::make_tuple(name, expr, location));
                }));

        ON_CALL(*this, setContent(testing::_)).WillByDefault(testing::Invoke([this](const std::string &content) {
            std::cout << "MockInvokeNode::setContent() called with: " << content << std::endl;
            this->content_ = content;
        }));

        ON_CALL(*this, setFinalize(testing::_)).WillByDefault(testing::Invoke([this](const std::string &finalize) {
