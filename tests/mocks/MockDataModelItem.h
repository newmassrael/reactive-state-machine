...
public:
    ...
    void SetupDefaultBehavior() {
        ...
        ON_CALL(*this, getExpr()).WillByDefault(testing::ReturnRef(expr_));
        ON_CALL(*this, getType()).WillByDefault(testing::ReturnRef(type_));
        ON_CALL(*this, getScope()).WillByDefault(testing::ReturnRef(scope_));
        ON_CALL(*this, getContent()).WillByDefault(testing::ReturnRef(content_));
        ON_CALL(*this, getSrc()).WillByDefault(testing::ReturnRef(src_));
        ON_CALL(*this, getAttributes()).WillByDefault(testing::ReturnRef(attributes_));
        ON_CALL(*this, getContentItems()).WillByDefault(testing::ReturnRef(contentItems_));
        ON_CALL(*this, isXmlContent()).WillByDefault(testing::Return(isXml_));

        // 메서드 호출 시 멤버 변수 업데이트 및 로깅 추가
        ON_CALL(*this, setExpr(testing::_)).WillByDefault(testing::Invoke([this](const std::string &expr) {
        ...
        ON_CALL(*this, setScope(testing::_)).WillByDefault(testing::Invoke([this](const std::string &scope) {
        ...
        ON_CALL(*this, addContent(testing::_)).WillByDefault(testing::Invoke([this](const std::string &content) {
        ...
        ON_CALL(*this, queryXPath(testing::_)).WillByDefault(testing::Invoke([this](const std::string &xpath) {
        ...
        ON_CALL(*this, supportsDataModel(testing::_))
            .WillByDefault(testing::Invoke([](const std::string &dataModelType) {
                std::cout << "supportsDataModel called with: " << dataModelType << std::endl;
                return (dataModelType == "xpath" || dataModelType == "xml" || dataModelType == "ecmascript" ||
                        dataModelType == "null");
            }));
        ON_CALL(*this, initialize(testing::_))
            .WillByDefault(testing::Invoke([](SCXML::Runtime::RuntimeContext &context) {
