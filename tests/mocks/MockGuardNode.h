...
public:
    ...
    void SetupDefaultBehavior() {
        // 함수 시작 로그
        std::cout << "MockGuardNode::SetupDefaultBehavior() - Setting up default behavior for guard: " << id_
                  << std::endl;
        ...
        ON_CALL(*this, isReactive()).WillByDefault(testing::Invoke([this]() {
        ...
        ON_CALL(*this, getAttribute(testing::_))
            .WillByDefault(testing::Invoke([this](const std::string &name) -> const std::string & {
        ...
        // 데이터 수정 메서드
        ON_CALL(*this, addDependency(testing::_)).WillByDefault(testing::Invoke([this](const std::string &dependency) {
            std::cout << "  - addDependency() called with: " << dependency << std::endl;
            dependencies_.push_back(dependency);
        }));

        ON_CALL(*this, setReactive(testing::_)).WillByDefault(testing::Invoke([this](bool reactive) {
            std::cout << "  - setReactive() called with: " << (reactive ? "true" : "false") << std::endl;
            reactive_ = reactive;
        }));

        ON_CALL(*this, setAttribute(testing::_, testing::_))
            .WillByDefault(testing::Invoke([this](const std::string &name, const std::string &value) {
        ...
        // 조건식 설정 메서드
        ON_CALL(*this, setCondition(testing::_)).WillByDefault(testing::Invoke([this](const std::string &condition) {
        ...
        ON_CALL(*this, setTargetState(testing::_))
            .WillByDefault(testing::Invoke([this](const std::string &targetState) {
