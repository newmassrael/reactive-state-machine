...
ExecutableContentParser::parseExecutableContent(const xmlpp::Element *element) {
    ...
    SCXML::Common::Logger::debug("ExecutableContentParser::parseExecutableContent - Parsing executable content in: " +
                                 element->get_name());
    ...
    for (const auto &child : element->get_children()) {
        ...
        if (childElement) {
            ...
            if (action) {
                ...
                SCXML::Common::Logger::debug("ExecutableContentParser::parseExecutableContent - Added action: " +
                                             action->getId());
            }
        }
    }

    SCXML::Common::Logger::debug("ExecutableContentParser::parseExecutableContent - Parsed " +
...
bool ExecutableContentParser::isSupportedAction(const xmlpp::Element *element) const {
    ...
    std::string tagName = element->get_name();
    return tagName == "if" || tagName == "send" || tagName == "raise" || tagName == "assign" || tagName == "log" ||
           tagName == "cancel" || tagName == "script" || tagName == "foreach";
}
...
ExecutableContentParser::parseRaiseAction(const xmlpp::Element *raiseElement) {
    if (!raiseElement) {
...
std::shared_ptr<SCXML::Model::IActionNode>
ExecutableContentParser::parseAssignAction(const xmlpp::Element *assignElement) {
    if (!assignElement) {
...
std::shared_ptr<SCXML::Model::IActionNode>
ExecutableContentParser::parseCancelAction(const xmlpp::Element *cancelElement) {
    if (!cancelElement) {
    ...
    if (!sendId.empty()) {
        ...
        SCXML::Common::Logger::error(
            "ExecutableContentParser::parseCancelAction - Missing 'sendid' or 'sendidexpr' attribute");
        return nullptr;
    }
...
std::shared_ptr<SCXML::Model::IActionNode>
ExecutableContentParser::parseScriptAction(const xmlpp::Element *scriptElement) {
    if (!scriptElement) {
...
std::shared_ptr<SCXML::Model::IActionNode>
ExecutableContentParser::parseForeachAction(const xmlpp::Element *foreachElement) {
    if (!foreachElement) {
    ...
    SCXML::Common::Logger::debug("ExecutableContentParser::parseForeachAction - Element name: " +
                                 foreachElement->get_name());

    auto foreachAction = std::make_shared<SCXML::Core::ForeachActionNode>(generateActionId("foreach"));

    // Parse foreach attributes
    std::string arrayVal = getAttributeValue(foreachElement, "array");
    std::string itemVal = getAttributeValue(foreachElement, "item");
    std::string indexVal = getAttributeValue(foreachElement, "index");

    SCXML::Common::Logger::debug("ExecutableContentParser::parseForeachAction - array=" + arrayVal);
    SCXML::Common::Logger::debug("ExecutableContentParser::parseForeachAction - item=" + itemVal);
    SCXML::Common::Logger::debug("ExecutableContentParser::parseForeachAction - index=" + indexVal);

    foreachAction->setArray(arrayVal);
    foreachAction->setItem(itemVal);
    foreachAction->setIndex(indexVal);
...
std::string ExecutableContentParser::getAttributeValue(const xmlpp::Element *element, const std::string &attributeName,
...
void ExecutableContentParser::parseConditionalStructure(std::shared_ptr<SCXML::Core::IfActionNode> ifAction,
    ...
    for (const auto &child : ifElement->get_children()) {
        ...
        if (tagName == "elseif") {
            ...
            if (condition.empty()) {
                SCXML::Common::Logger::error(
                    "ExecutableContentParser::parseConditionalStructure - Missing 'cond' in elseif");
                continue;
            }
            ...
            SCXML::Common::Logger::debug("ExecutableContentParser::parseConditionalStructure - Added elseif branch: " +
                                         condition);
            ...
            if (action) {
                ...
                SCXML::Common::Logger::debug(
                    "ExecutableContentParser::parseConditionalStructure - Added action to if branch: " +
                    action->getId());
            }
        }
    }
...
std::vector<std::shared_ptr<SCXML::Model::IActionNode>>
ExecutableContentParser::parseChildActions(const xmlpp::Element *parent) {
    std::vector<std::shared_ptr<SCXML::Model::IActionNode>> actions;
