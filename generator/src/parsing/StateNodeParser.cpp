...
StateNodeParser::StateNodeParser(std::shared_ptr<SCXML::Model::INodeFactory> nodeFactory) : nodeFactory_(nodeFactory) {
    SCXML::Common::Logger::debug("StateNodeParser::Constructor - Creating state node parser");
}

StateNodeParser::~StateNodeParser() {
    SCXML::Common::Logger::debug("StateNodeParser::Destructor - Destroying state node parser");
}

void StateNodeParser::setRelatedParsers(std::shared_ptr<TransitionParser> transitionParser,
    ...
    SCXML::Common::Logger::debug("StateNodeParser::setRelatedParsers() - Related parsers set");
}

std::shared_ptr<::SCXML::Model::IStateNode>
StateNodeParser::parseStateNode(const xmlpp::Element *stateElement,
                                std::shared_ptr<::SCXML::Model::IStateNode> parentState,
                                const SCXML::Model::ParsingContext &context) {
    if (!stateElement) {
        SCXML::Common::Logger::warning("StateNodeParser::parseStateNode() - Null state element");
        return nullptr;
    }
    ...
    if (idAttr) {
        ...
        // ID가 없는 경우 자동 생성
        static std::atomic<uint32_t> stateIdCounter{1};
        stateId = "state_" + std::to_string(stateIdCounter.fetch_add(1));
        SCXML::Common::Logger::warning("StateNodeParser::parseStateNode() - State has no ID, generated: " + stateId);
    }
    ...
    if (!stateNode) {
        SCXML::Common::Logger::error("StateNodeParser::parseStateNode() - Failed to create state node");
        return nullptr;
    }
    ...
    if (!parentState) {
        SCXML::Common::Logger::debug("StateNodeParser::parseStateNode() - No parent state (root)");
    }

    // 히스토리 상태인 경우 추가 처리
    if (stateType == SCXML::Type::HISTORY) {
    ...
    if (stateType != SCXML::Type::FINAL && stateType != SCXML::Type::HISTORY) {
    ...
    if (stateType == SCXML::Type::FINAL && doneDataParser_) {
        const xmlpp::Element *doneDataElement =
            SCXML::Parsing::ParsingCommon::findFirstChildElement(stateElement, "donedata");
        if (doneDataElement) {
    ...
    if (stateType == SCXML::Type::COMPOUND && !stateNode->getChildren().empty()) {
        if (stateType == SCXML::Type::COMPOUND && !stateNode->getChildren().empty()) {
            // <initial> 요소 확인
            auto initialElement = SCXML::Parsing::ParsingCommon::findFirstChildElement(stateElement, "initial");
            if (initialElement) {
                // <initial> 요소 파싱
...
SCXML::Type StateNodeParser::determineStateType(const xmlpp::Element *stateElement) {
    if (!stateElement) {
        return SCXML::Type::ATOMIC;
    }
    ...
    // history 요소 확인
    if (SCXML::Parsing::ParsingCommon::matchNodeName(nodeName, "history")) {
        return SCXML::Type::HISTORY;
    }

    // final 요소 확인
    if (SCXML::Parsing::ParsingCommon::matchNodeName(nodeName, "final")) {
        return SCXML::Type::FINAL;
    }

    // parallel 요소 확인
    if (SCXML::Parsing::ParsingCommon::matchNodeName(nodeName, "parallel")) {
        return SCXML::Type::PARALLEL;
    }
    ...
    for (auto child : children) {
        ...
        if (element) {
            ...
            if (SCXML::Parsing::ParsingCommon::matchNodeName(childName, "state") ||
                SCXML::Parsing::ParsingCommon::matchNodeName(childName, "parallel") ||
                SCXML::Parsing::ParsingCommon::matchNodeName(childName, "final") ||
                SCXML::Parsing::ParsingCommon::matchNodeName(childName, "history")) {
                hasChildStates = true;
                break;
            }
        }
...
void StateNodeParser::parseTransitions(const xmlpp::Element *parentElement,
                                       std::shared_ptr<::SCXML::Model::IStateNode> state) {
    if (!parentElement || !state || !transitionParser_) {
        return;
    }

    auto transitionElements = SCXML::Parsing::ParsingCommon::findChildElements(parentElement, "transition");
...
void StateNodeParser::parseEntryExitElements(const xmlpp::Element *parentElement,
    ...
    // onentry 요소 처리
    auto onentryElements = SCXML::Parsing::ParsingCommon::findChildElements(parentElement, "onentry");
    ...
    // onexit 요소 처리
    auto onexitElements = SCXML::Parsing::ParsingCommon::findChildElements(parentElement, "onexit");
...
void StateNodeParser::parseChildStates(const xmlpp::Element *stateElement,
                                       std::shared_ptr<::SCXML::Model::IStateNode> parentState,
                                       const SCXML::Model::ParsingContext &context) {
    SCXML::Common::Logger::debug("StateNodeParser::parseChildStates() - Parsing child states");

    // state, parallel, final, history 등의 자식 요소 검색
    std::vector<const xmlpp::Element *> childStateElements;
    auto stateElements = SCXML::Parsing::ParsingCommon::findChildElements(stateElement, "state");
    childStateElements.insert(childStateElements.end(), stateElements.begin(), stateElements.end());

    auto parallelElements = SCXML::Parsing::ParsingCommon::findChildElements(stateElement, "parallel");
    childStateElements.insert(childStateElements.end(), parallelElements.begin(), parallelElements.end());

    auto finalElements = SCXML::Parsing::ParsingCommon::findChildElements(stateElement, "final");
    childStateElements.insert(childStateElements.end(), finalElements.begin(), finalElements.end());

    auto historyElements = SCXML::Parsing::ParsingCommon::findChildElements(stateElement, "history");
...
void StateNodeParser::parseInvokeElements(const xmlpp::Element *parentElement,
    ...
    auto invokeElements = SCXML::Parsing::ParsingCommon::findChildElements(parentElement, "invoke");
...
void StateNodeParser::parseReactiveGuards(const xmlpp::Element *parentElement,
                                          std::shared_ptr<::SCXML::Model::IStateNode> state) {
    ...
    auto reactiveGuardElements = SCXML::Parsing::ParsingCommon::findChildElementsWithNamespace(
    ...
    for (auto *reactiveGuardElement : reactiveGuardElements) {
        ...
        if (idAttr) {
            ...
            SCXML::Common::Logger::debug("StateNodeParser::parseReactiveGuards() - Added reactive guard: " + guardId);
        } else {
            SCXML::Common::Logger::warning("StateNodeParser::parseReactiveGuards() - Reactive guard without ID");
        }
    }
...
void StateNodeParser::parseInitialElement(const xmlpp::Element *initialElement,
    ...
    const xmlpp::Element *transitionElement =
        SCXML::Parsing::ParsingCommon::findFirstChildElement(initialElement, "transition");
    if (transitionElement) {
        // 전환 파싱 - 부모 상태도 함께 전달
        auto transition = transitionParser_->parseTransitionNode(transitionElement, state.get());
        if (transition) {
            // ::SCXML::Model::IStateNode 인터페이스를 통해 직접 호출
