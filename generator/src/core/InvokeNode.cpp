...
InvokeNode::InvokeNode(const std::string &id) : id_(id), autoForward_(false) {
    SCXML::Common::Logger::debug("InvokeNode::Constructor - Creating invoke node: " + id);
}

InvokeNode::~InvokeNode() {
    SCXML::Common::Logger::debug("InvokeNode::Destructor - Destroying invoke node: " + id_);
}
...
void InvokeNode::setType(const std::string &type) {
    SCXML::Common::Logger::debug("InvokeNode::setType() - Setting type for " + id_ + ": " + type);
    type_ = type;
}

void InvokeNode::setSrc(const std::string &src) {
    SCXML::Common::Logger::debug("InvokeNode::setSrc() - Setting src for " + id_ + ": " + src);
    src_ = src;
}

void InvokeNode::setAutoForward(bool autoForward) {
    SCXML::Common::Logger::debug("InvokeNode::setAutoForward() - Setting autoForward for " + id_ + ": " +
                  (autoForward ? "true" : "false"));
    autoForward_ = autoForward;
}

void InvokeNode::setIdLocation(const std::string &idLocation) {
    SCXML::Common::Logger::debug("InvokeNode::setIdLocation() - Setting idLocation for " + id_ + ": " + idLocation);
    idLocation_ = idLocation;
}

void InvokeNode::setNamelist(const std::string &namelist) {
    SCXML::Common::Logger::debug("InvokeNode::setNamelist() - Setting namelist for " + id_ + ": " + namelist);
    namelist_ = namelist;
}

void InvokeNode::addParam(const std::string &name, const std::string &expr, const std::string &location) {
    SCXML::Common::Logger::debug("InvokeNode::addParam() - Adding param to " + id_ + ": name=" + name);
    params_.push_back(std::make_tuple(name, expr, location));
}

void InvokeNode::setContent(const std::string &content) {
    SCXML::Common::Logger::debug("InvokeNode::setContent() - Setting content for " + id_);
    content_ = content;
}

void InvokeNode::setFinalize(const std::string &finalizeContent) {
    SCXML::Common::Logger::debug("InvokeNode::setFinalize() - Setting finalize for " + id_);
