...
SCXML::GuardEvaluator::GuardEvaluator() {
    initializeDefaultEvaluator();
}

SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateTransitionGuard(
    ...
    try {
        ...
        result.satisfied = true;
        SCXML::Common::Logger::debug("GuardEvaluator::evaluateTransitionGuard - No evaluable guard found for transition");
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateGuardNode(
    std::shared_ptr<SCXML::Model::IGuardNode> guard,
    const GuardContext& context) {
    ...
    try {
        ...
        SCXML::Common::Logger::debug("GuardEvaluator::evaluateGuardNode - Evaluating guard node condition: '" + condition + "'");
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateExpression(
    ...
    try {
        ...
        if (expression.find("&&") != std::string::npos || 
            ...
            (expression.find("!") == 0 && expression.length() > 1) ||
            expression.find("In(") != std::string::npos) {
        ...
        if (context.runtimeContext) {
            ...
            if (context.runtimeContext->hasDataValue(expression)) {
                ...
                SCXML::Common::Logger::debug("GuardEvaluator::evaluateExpression - Variable " + expression + " = " + value);
                return result;
            }
            
            // Check for equality expressions: "variable == 'value'"
            std::regex eqPattern(R"((\w+)\s*==\s*['\"]([^'\"]*)['\"])");
            std::smatch matches;
            if (std::regex_match(expression, matches, eqPattern)) {
                ...
                if (context.runtimeContext->hasDataValue(varName)) {
                    ...
                    SCXML::Common::Logger::debug("GuardEvaluator::evaluateExpression - " + varName + " (" + actualValue + ") == '" + expectedValue + "' -> " + (result.satisfied ? "true" : "false"));
                    return result;
                }
            }
            ...
            if (std::regex_match(expression, matches, eventPattern)) {
                ...
                if (context.currentEvent) {
                    ...
                    SCXML::Common::Logger::debug("GuardEvaluator::evaluateExpression - _event.name (" + actualEventName + ") == '" + expectedEventName + "' -> " + (result.satisfied ? "true" : "false"));
                    return result;
                }
            }
        }
        ...
        if (expressionEvaluator_) {
            ...
            if (evalResult.success) {
                ...
                SCXML::Common::Logger::debug(std::string("GuardEvaluator::evaluateExpression - Expression evaluator result: ") + (result.satisfied ? "true" : "false"));
                return result;
            } else {
        ...
        SCXML::Common::Logger::warning("GuardEvaluator::evaluateExpression - Cannot evaluate expression '" + expression + "', assuming true");
        result.satisfied = true;
...
SCXML::Runtime::ExpressionEvaluator::EventContext SCXML::GuardEvaluator::createEvaluationContext(
    ...
    if (guardCtx.runtimeContext) {
        ...
        evalCtx.eventFields["__runtime_context__"] = std::to_string(
            reinterpret_cast<uintptr_t>(guardCtx.runtimeContext)
        );
    ...
    if (guardCtx.currentEvent) {
        ...
        // evalCtx.eventFields["_event.type"] = std::to_string(static_cast<int>(guardCtx.currentEvent->getType())); // Incomplete type issue
...
bool SCXML::GuardEvaluator::isValidGuardExpression(const std::string& expression) const {
    ...
    if (expression.find("//") != std::string::npos) return false; // No double slashes
    if (expression.find("/*") != std::string::npos) return false; // No comments
    
    return true;
}

std::vector<std::string> SCXML::GuardEvaluator::getAllGuardExpressions(
    std::shared_ptr<SCXML::Model::ITransitionNode> transition) const {
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateMultipleExpressions(
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::handleError(
    const std::string& expression,
...
void SCXML::GuardEvaluator::initializeDefaultEvaluator() {
    ...
    SCXML::Common::Logger::info("GuardEvaluator::initializeDefaultEvaluator - Initialized with basic ExpressionEvaluator");
}
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateComplexExpression(
    const std::string& expression,
    const GuardContext& context) {
    ...
    try {
        ...
        for (size_t i = 0; i < trimmed.length() - 1; ++i) {
            if (trimmed[i] == '(') parenLevel++;
            else if (trimmed[i] == ')') parenLevel--;
        ...
        if (context.runtimeContext && context.runtimeContext->hasDataValue(trimmed)) {
            return evaluateDataModelGuard(trimmed, context);
        }
        
        // Fall back to basic expression evaluation
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateEventGuard(
    const std::string& expression,
    const GuardContext& context) {
                ...
                if (!rightSide.empty() && (rightSide[0] == '"' || rightSide[0] == '\'')) {
                    ...
                    if (endQuote != std::string::npos) {
                        ...
                        SCXML::Common::Logger::debug("GuardEvaluator::evaluateEventGuard - Event name (" + eventName + 
                                     ") == '" + expectedName + "' -> " + (result.satisfied ? "true" : "false"));
                        return result;
                    }
                }
            }
                ...
                if (!rightSide.empty() && (rightSide[0] == '"' || rightSide[0] == '\'')) {
                    ...
                    if (endQuote != std::string::npos) {
                        ...
                        SCXML::Common::Logger::debug("GuardEvaluator::evaluateEventGuard - Event data (" + dataValue + 
                                     ") == '" + expectedData + "' -> " + (result.satisfied ? "true" : "false"));
                        return result;
                    }
                }
            }
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateStateGuard(
    const std::string& expression,
    const GuardContext& context) {
...
SCXML::GuardEvaluator::GuardResult SCXML::GuardEvaluator::evaluateDataModelGuard(
    const std::string& expression,
    const GuardContext& context) {
                ...
                if (context.runtimeContext->hasDataValue(varName)) {
                    ...
                    if (op == "==") {
                        ...
                        try {
                            ...
                            if (op == "<") compResult = (leftNum < rightNum);
                            else if (op == ">") compResult = (leftNum > rightNum);
                            else if (op == "<=") compResult = (leftNum <= rightNum);
                            else if (op == ">=") compResult = (leftNum >= rightNum);
                        } catch (const std::exception&) {
                            // Fall back to string comparison
                            if (op == "<") compResult = (leftValue < rightSide);
                            else if (op == ">") compResult = (leftValue > rightSide);
                            else if (op == "<=") compResult = (leftValue <= rightSide);
                            else if (op == ">=") compResult = (leftValue >= rightSide);
                        }
                    }
                    ...
                    SCXML::Common::Logger::debug("GuardEvaluator::evaluateDataModelGuard - " + varName + " (" + leftValue + 
                                 ") " + op + " " + rightSide + " -> " + (result.satisfied ? "true" : "false"));
                    return result;
                }
