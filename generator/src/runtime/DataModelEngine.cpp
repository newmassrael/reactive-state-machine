...
public:
    DataModelEngine::DataValue evaluate(const std::string& expression, 
        ...
        if (expression == "null" || expression == "undefined") return std::monostate{};
...
private:
    DataModelEngine::DataValue parseArrayLiteral(const std::string& arrayExpr,
                                                  const std::unordered_map<std::string, DataModelEngine::DataValue>& variables) {
    ...
    DataModelEngine::DataValue parseObjectLiteral(
    ...
    DataModelEngine::DataValue evaluatePropertyAccess(const std::string& expression,
                                                      const std::unordered_map<std::string, DataModelEngine::DataValue>& variables) {
    ...
    DataModelEngine::DataValue evaluateArrayAccess(const std::string& expression,
                                                   const std::unordered_map<std::string, DataModelEngine::DataValue>& variables) {
        ...
        if (arrayIt != variables.end() && std::holds_alternative<std::shared_ptr<DataModelEngine::DataArray>>(arrayIt->second)) {
    ...
    DataModelEngine::DataValue evaluateSimpleExpression(const std::string& expression,
                                                        const std::unordered_map<std::string, DataModelEngine::DataValue>& variables) {
        // Handle simple arithmetic and comparisons
        std::vector<std::pair<std::string, std::function<DataModelEngine::DataValue(const DataModelEngine::DataValue&, const DataModelEngine::DataValue&)>>> operators = {
    ...
    static DataModelEngine::DataValue addValues(const DataModelEngine::DataValue& left, const DataModelEngine::DataValue& right) {
    ...
    static DataModelEngine::DataValue subtractValues(const DataModelEngine::DataValue& left, const DataModelEngine::DataValue& right) {
        try {
            double leftNum = std::stod(valueToString(left));
            double rightNum = std::stod(valueToString(right));
            return leftNum - rightNum;
    ...
    DataModelEngine::DataValue evaluate(const std::string& expression, 
        ...
        try {
            ...
            xmlXPathObjectPtr xpathObj = xmlXPathEvalExpression(
                BAD_CAST expression.c_str(), xpathCtx_);
...
private:
    ...
    DataModelEngine::DataValue convertXPathResult(xmlXPathObjectPtr xpathObj) {
        ...
        switch (xpathObj->type) {
            ...
            default:
                SCXML::Common::Logger::warning("XPathEvaluator: Unsupported XPath result type: " + std::to_string(xpathObj->type));
                return std::monostate{};
        }
    }
    
    // Fallback evaluation for simple expressions when libxml2 XPath fails
    DataModelEngine::DataValue fallbackEvaluation(const std::string& expression, 
                                                 const std::unordered_map<std::string, DataModelEngine::DataValue>& variables) {
...
struct DataModelEngine::EvaluatorImpl {
    ...
    std::unique_ptr<IECMAScriptEngine> ecmaScriptEngine;
...
void DataModelEngine::initializeTypeConverters() {
    typeConverters_["string"] = [](const std::string& str) -> DataValue {
        return str;
    };
    ...
    for (const auto& item : dataItems) {
        ...
        if (!expr.empty()) {
            ...
            if (result.success) {
                ...
                SCXML::Common::Logger::warning("Failed to evaluate expression for data item '" + id + "': " + result.errorMessage);
...
void DataModelEngine::clear(std::optional<Scope> scope) {
    ...
    std::string scopeStr = scope ? (scope == Scope::GLOBAL ? "global" :
                          scope == Scope::LOCAL ? "local" : "session") : "all";
    SCXML::Common::Logger::debug("Cleared data model scope: " + scopeStr);
}

DataModelEngine::DataModelResult 
DataModelEngine::setValue(const std::string& location, const DataValue& value, Scope scope) {
...
DataModelEngine::DataModelResult 
DataModelEngine::getValue(const std::string& location, std::optional<Scope> scope) const {
    ...
    for (auto searchScope : scopes) {
        ...
        // 🔍 CRITICAL: Check if this navigateToLocation call triggers JavaScript
...
DataModelEngine::DataModelResult 
DataModelEngine::removeValue(const std::string& location, std::optional<Scope> scope) {
    auto path = parseLocation(location);
    if (!path.isValid) {
        return DataModelResult::createError("Invalid location path: " + location);
    }
    ...
    for (auto searchScope : scopes) {
        ...
        if (path.segments.size() == 1) {
            ...
            DataValue* parent = navigateToLocation(storage, 
                LocationPath{},
                false);
...
DataModelEngine::DataModelResult DataModelEngine::evaluateExpression(const std::string& expression, SCXML::Runtime::RuntimeContext& context) {
    ...
    if (evaluationStack.count(expression) > 0) {
        SCXML::Common::Logger::error("DataModelEngine::evaluateExpression - Circular reference detected: " + expression);
    ...
    if (evaluationStack.size() >= MAX_EVALUATION_DEPTH) {
        SCXML::Common::Logger::error("DataModelEngine::evaluateExpression - Max evaluation depth exceeded: " + std::to_string(evaluationStack.size()));
    ...
    struct EvaluationGuard {
        ...
        ~EvaluationGuard() { evaluationStack.erase(expr); }
    };
    ...
    try {
        ...
        for (const auto& pair : sessionData_) allVariables[pair.first] = pair.second;
        for (const auto& pair : globalData_) allVariables[pair.first] = pair.second;
        for (const auto& pair : localData_) allVariables[pair.first] = pair.second;
        ...
        switch (dataModelType_) {
            case DataModelType::ECMASCRIPT:
                ...
                if (!evaluatorImpl_->ecmaScriptEngine) {
                    // Create new instance (will be optimized by singleton pattern inside factory)
                    evaluatorImpl_->ecmaScriptEngine = ECMAScriptEngineFactory::create(ECMAScriptEngineFactory::EngineType::AUTO);
                    if (evaluatorImpl_->ecmaScriptEngine && !evaluatorImpl_->ecmaScriptEngine->initialize()) {
                        SCXML::Common::Logger::error("DataModelEngine::evaluateExpression - Failed to initialize ECMAScript engine");
                    ...
                    if (evalResult.success) {
                        ...
                        std::visit([&result](const auto& v) {
                            ...
                            if constexpr (std::is_same_v<T, std::monostate>) {
                                ...
                                result = std::string(v ? "true" : "false");
                        ...
                        SCXML::Common::Logger::error("DataModelEngine: ECMAScript engine evaluation failed: " + evalResult.errorMessage);
                        result = std::string("null");
                    }
                    
                    SCXML::Common::Logger::debug("DataModelEngine: ECMAScript engine evaluation result: " + valueToString(result));
                } else {
...
#else
                SCXML::Common::Logger::error("DataModelEngine: XPath evaluator not available (compiled without HAS_LIBXML2)");
                result = std::string("XPath evaluator not available");
#endif
                break;
...
DataModelEngine::DataModelResult DataModelEngine::executeScript(const std::string& script, SCXML::Runtime::RuntimeContext& context) {
    if (script.empty()) {
        return DataModelResult::createSuccess();
    }
    ...
    try {
        ...
        switch (dataModelType_) {
            case DataModelType::ECMASCRIPT:
                ...
                if (!evaluatorImpl_->ecmaScriptEngine) {
                    ...
                    evaluatorImpl_->ecmaScriptEngine = ECMAScriptEngineFactory::create(ECMAScriptEngineFactory::EngineType::AUTO);
                    if (evaluatorImpl_->ecmaScriptEngine && !evaluatorImpl_->ecmaScriptEngine->initialize()) {
                        SCXML::Common::Logger::error("DataModelEngine::executeScript - Failed to initialize ECMAScript engine");
                        evaluatorImpl_->ecmaScriptEngine.reset();
                    }
                }
                ...
                if (evaluatorImpl_->ecmaScriptEngine) {
                    ...
                    if (execResult.success) {
                        ...
                        SCXML::Common::Logger::warning("DataModelEngine::executeScript - Script execution failed: " + execResult.errorMessage);
                        // Return success to allow graceful continuation
                        return DataModelResult::createSuccess(std::string("Script execution failed but continued gracefully"));
                    }
                } else {
                    SCXML::Common::Logger::warning("DataModelEngine::executeScript - ECMAScript engine not initialized");
                    return DataModelResult::createSuccess(std::string("Script engine not available but continued gracefully"));
                }
                break;
                
            case DataModelType::XPATH:
                // XPath doesn't support script execution
                SCXML::Common::Logger::warning("DataModelEngine::executeScript - Script execution not supported in XPath data model");
            ...
            case DataModelType::NULL_MODEL:
                ...
                SCXML::Common::Logger::debug("DataModelEngine::executeScript - Script execution disabled in null data model");
        ...
        SCXML::Common::Logger::warning("DataModelEngine::executeScript - Exception during script execution: " + std::string(e.what()));
        // Return success to allow graceful continuation
...
DataModelEngine::DataValue DataModelEngine::parseValue(const std::string& str) const {
    ...
    if (str == "null") return std::monostate{};
...
std::unordered_map<std::string, DataModelEngine::DataValue>& 
DataModelEngine::getDataStorage(Scope scope) {
    switch (scope) {
        case Scope::GLOBAL: return globalData_;
        case Scope::LOCAL: return localData_;
        case Scope::SESSION: return sessionData_;
        default: return globalData_;
    }
}

const std::unordered_map<std::string, DataModelEngine::DataValue>& 
DataModelEngine::getDataStorage(Scope scope) const {
    switch (scope) {
        case Scope::GLOBAL: return globalData_;
        case Scope::LOCAL: return localData_;
        case Scope::SESSION: return sessionData_;
...
DataModelEngine::navigateToLocation(std::unordered_map<std::string, DataValue>& storage,
...
DataModelEngine::DataValue* 
DataModelEngine::navigateNestedValue(DataValue& value, const std::string& key, 
        ...
        if (std::holds_alternative<std::shared_ptr<DataArray>>(value)) {
            ...
            if (arrayIndex.value() < array.size()) {
                ...
                while (array.size() <= arrayIndex.value()) {
                    array.push_back(DataValue{std::monostate{}});
                }
                return &array[arrayIndex.value()];
            }
            ...
            while (newArray.size() <= arrayIndex.value()) {
                newArray.push_back(DataValue{std::monostate{}});
            }
    ...
    if (std::holds_alternative<std::shared_ptr<DataObject>>(value)) {
        ...
        newObject[key] = DataValue{std::monostate{}};
...
const DataModelEngine::DataValue* 
DataModelEngine::navigateNestedValue(const DataValue& value, const std::string& key) const {
    // Try array access first
    auto arrayIndex = parseArrayIndex(key);
    if (arrayIndex.has_value()) {
        if (std::holds_alternative<std::shared_ptr<DataArray>>(value)) {
...
bool DataModelEngine::setNestedValue(DataValue& value, const std::string& key, const DataValue& newValue) {
    ...
    auto arrayIndex = parseArrayIndex(key);
    if (arrayIndex.has_value()) {
        if (!ensureObjectForNesting(value)) return false;
...
bool DataModelEngine::ensureObjectForNesting(DataValue& value) {
    if (std::holds_alternative<std::shared_ptr<DataObject>>(value) || std::holds_alternative<std::shared_ptr<DataArray>>(value)) {
...
void DataModelEngine::logOperation(const std::string& operation, 
...
int64_t DataModelEngine::DataArray::indexOf(const DataValue& value) const {
    for (size_t i = 0; i < elements_.size(); ++i) {
        ...
        std::string thisStr = std::visit([](const auto& v) -> std::string {
            using T = std::decay_t<decltype(v)>;
            if constexpr (std::is_same_v<T, std::monostate>) {
                return "null";
            } else if constexpr (std::is_same_v<T, bool>) {
...
std::string DataModelEngine::DataArray::toString() const {
    ...
    for (const auto& item : elements_) {
        ...
        std::string itemStr = std::visit([](const auto& v) -> std::string {
            using T = std::decay_t<decltype(v)>;
            if constexpr (std::is_same_v<T, std::monostate>) {
                return "null";
            } else if constexpr (std::is_same_v<T, bool>) {
...
void DataModelEngine::pushScope(const std::string& scopeId) {
    ...
    SCXML::Common::Logger::debug("DataModel: Pushed new scope '" + scopeIds_.back() + "' (depth: " + std::to_string(scopeStack_.size()) + ")");
}
...
bool DataModelEngine::popScope() {
    ...
    SCXML::Common::Logger::debug("DataModel: Popped scope '" + scopeId + "' (depth: " + std::to_string(scopeStack_.size()) + ")");
    return true;
}
...
DataModelEngine::DataModelResult 
DataModelEngine::setValueWithInheritance(const std::string& location,
...
DataModelEngine::DataModelResult 
DataModelEngine::getValueWithInheritance(const std::string& location,
    ...
    for (auto scope : searchOrder) {
        ...
        if (value) {
            logOperation("GET_INHERITED", location + "@" + 
                        (scope == Scope::GLOBAL ? "global" : 
                         scope == Scope::LOCAL ? "local" : "session"), true);
            return DataModelResult::createSuccess(*value);
        }
    }
...
std::optional<DataModelEngine::Scope> 
DataModelEngine::findVariableScope(const std::string& location,
                                  const std::vector<Scope>& searchOrder) {
...
size_t DataModelEngine::copyScope(Scope fromScope, Scope toScope, const std::string& filter) {
    ...
    SCXML::Common::Logger::debug("DataModel: Copied " + std::to_string(copiedCount) + " variables from " +
                 (fromScope == Scope::GLOBAL ? "global" : 
                  fromScope == Scope::LOCAL ? "local" : "session") + " to " +
                 (toScope == Scope::GLOBAL ? "global" : 
                  toScope == Scope::LOCAL ? "local" : "session"));
...
size_t DataModelEngine::onDataChange(const std::string& location,
                                    DataChangeCallback callback,
                                    std::optional<Scope> scope) {
    size_t id = nextCallbackId_++;
    dataChangeListeners_.push_back({id, location, callback, scope});
    
    SCXML::Common::Logger::debug("DataModel: Registered data change listener " + std::to_string(id) + " for '" + location + "'");
    return id;
}
...
bool DataModelEngine::removeDataChangeCallback(size_t callbackId) {
    auto it = std::find_if(dataChangeListeners_.begin(), dataChangeListeners_.end(),
                          [callbackId](const DataChangeListener& listener) {
...
size_t DataModelEngine::createBinding(const std::string& location1, const std::string& location2,
                                     Scope scope1, Scope scope2) {
    size_t id = nextCallbackId_++;
    dataBindings_.push_back({id, location1, location2, scope1, scope2, false});
    
    SCXML::Common::Logger::debug("DataModel: Created binding " + std::to_string(id) + " between '" + 
...
bool DataModelEngine::removeBinding(size_t bindingId) {
    auto it = std::find_if(dataBindings_.begin(), dataBindings_.end(),
                          [bindingId](const DataBinding& binding) {
...
size_t DataModelEngine::createComputed(const std::string& location,
    ...
    SCXML::Common::Logger::debug("DataModel: Created computed property " + std::to_string(id) + " at '" + 
                 location + "' with " + std::to_string(dependencies.size()) + " dependencies");
    return id;
}
...
bool DataModelEngine::removeComputed(size_t computedId) {
    auto it = std::find_if(computedProperties_.begin(), computedProperties_.end(),
                          [computedId](const ComputedProperty& computed) {
                              return computed.id == computedId;
                          });
...
bool DataModelEngine::matchesPattern(const std::string& location, const std::string& pattern) const {
    ...
    if (pattern.find('*') == std::string::npos) return location == pattern;
...
void DataModelEngine::notifyDataChange(const std::string& location,
                                      const DataValue& oldValue,
...
void DataModelEngine::updateBindings(const std::string& location, const DataValue& newValue, Scope scope) {
    for (auto& binding : dataBindings_) {
        if (binding.updating) continue; // Prevent circular updates
