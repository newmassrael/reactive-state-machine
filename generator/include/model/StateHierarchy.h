...
public:
     ...
     */
    void setRootState(std::shared_ptr<Model::IStateNode> rootState);

    /**
     * @brief 루트 상태 반환
     * @return 루트 상태 노드
     */
    Model::IStateNode *getRootState() const;
     ...
     * @return 성공 여부
     */
    bool addState(std::shared_ptr<Model::IStateNode> state, const std::string &parentId = "");
     ...
     */
    Model::IStateNode *findStateById(const std::string &id) const;
     ...
     * @return 모든 상태 노드 목록
     */
    const std::vector<std::shared_ptr<Model::IStateNode>> &getAllStates() const;
...
private:
     ...
     * @param depth 깊이
     */
    void printStateHierarchy(Model::IStateNode *state, int depth) const;
     ...
     */
    bool isDescendantOf(Model::IStateNode *ancestor, Model::IStateNode *descendant) const;

    // 멤버 변수
    std::shared_ptr<Model::IStateNode> rootState_;
    std::vector<std::shared_ptr<Model::IStateNode>> allStates_;
    std::unordered_map<std::string, Model::IStateNode *> stateIdMap_;
};
