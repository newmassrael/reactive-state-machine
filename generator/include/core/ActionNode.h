...
public:
     ...
     * @return 실행 성공 여부
     */
    virtual bool execute(::SCXML::Runtime::RuntimeContext &context) override;

    /**
     * @brief 액션 노드 복제 (기본 구현)
     * @return 복제된 액션 노드
     */
    virtual std::shared_ptr<IActionNode> clone() const override;

    /**
     * @brief 액션 타입 이름 반환 (기본 구현)
     * @return 액션 타입 문자열 ("unknown" for base class)
     */
    virtual std::string getActionType() const override {
        return "unknown";
    }
